...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: QLegacy.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: QLegacy.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeQLegacyamount=<n> " + _("Keep N QLegacy anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. QLegacy is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. QLegacy is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("QLegacy version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "QLegacy server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of QLegacy") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart QLegacy to complete") << "\n";
    ...
    nAnonymizeQLegacyAmount = GetArg("-anonymizeQLegacyamount", 0);
    if(nAnonymizeQLegacyAmount > 999999) nAnonymizeQLegacyAmount = 999999;
    if(nAnonymizeQLegacyAmount < 2) nAnonymizeQLegacyAmount = 2;
    ...
    LogPrintf("Anonymize QLegacy Amount %d\n", nAnonymizeQLegacyAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1QXAN+1000 == (.1QXAN+100)*10
       10QXAN+10000 == (1QXAN+1000)*10
    */
