...
{
    // Convert QLegacy:// to QLegacy:
    //
    //    Cannot handle this later, because QLegacy:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("QLegacy://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "QLegacy:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "QLegacy.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for QLegacy.lnk
...
{
    return GetAutostartDir() / "QLegacy.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a QLegacy.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=QLegacy\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the QLegacy app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add QLegacy app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("QLegacy-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  QLegacy-qt [" + tr("command-line options") + "]                     " + "\n";
